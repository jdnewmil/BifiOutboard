...
author:
  - id: jdnewmil
    ...
    attributes:
      corresponding: true
abstract: |
  This paper presents a strategy for evaluating photovoltaic (PV) system power capacity while avoiding sensor placement within localized rear-side irradiance gradients by comparison with a computed estimate of sensor readings based on existing PVsyst model outputs.  
keywords: [ASTM2848, system capacity testing, bifacial photovoltaic]
#funding: 
pageheader:
  left: PVSC 52 Abstract, June 2024
  right: 'J. Newmiller:  Bifacial ASTM2848 Capacity Testing with Outboard Sensors'
...
[C]{.IEEEPARstart}[haracterization]{} of PV power generating system capacity using the ASTM2848 test procedure is widely implemented in accptance test protocols for photovoltaic power systems. However, this procedure was developed and tested in the last century when photovoltaic modules were designed to only convert irradiant power incident on one side (monofacial). This construction allowed a single pyranometer to be represent the average incident irradiation available to the PV module array for most cases, which in turn allowed the total input power to the array (before internal shading) to be quantified in the field by an assumed multiplication of sum of module area with measured irradiance. While actual field conditions might vary such that any particular set of input conditions (POA irradiance, ambient temperature, and wind speed) do not occur during an equipment evaluation or in a simulated equipment operation, a regression can be defined that allows exactly the same input conditions to be specified identically for the simulated performance and the observed performance. The irradiance measurement typically associated with such PV system capacity tests are adjacent to and coplanar with the plane of modules in a row within an array in a location with minimal shade impact. The rationale for using this independent variable is that the power prediction model should be able to properly predict the power reduction due to diffuse shading, but it is impractical to compare the model to field conditions when there is direct irradiance shading on the array so such records are excluded from the evaluation data.

The widespread introduction of bifacial PV modules to the market in the 2010s lead to a variety of approaches to quantify the additional input power from the rear side of the modules, but the key assumption that a pyranometer could be placed in a location that would be representative for the whole rear side of the PV module array is difficult to defend. [@mcintoshEffectNonuniformIllumination2019] demonstrated that the incident rear-side irradiance on single-axis trackers is far from uniform on the rear side of the PV module array, and that the non-uniformity varies with time as well. That is, any specific location that might be representative of the spatial average rear side irradiance value at one time of day may not be relied on to serve that purpose at another time of day. This particularly makes identification of excessive rear-side structural shading difficult, since mounting a pyranometer in the plane of the rear transparent cover between two module may lead to an excessively-low estimate of rear-side irradiance leading to an over-estimate of power plant power conversion capacity. Moving the pyranometer until it is coplanar with the purlins/module frame will avoid the overly-conservative shaded measurement but then excessive torque tube and support shading will still not be identified.

[@pelaezEffectTorquetubeParameters2019] suggested placement of rear facing sensors at 20% and 80% positions along a SAT row transverse dimension and using the average of these two sensors to estimate average irradiance intensity. This approach addresses long-term average spatial variations, but does not address the short-term spatial irradiance change with time of day illustrated by [@mcintoshEffectNonuniformIllumination2019] that would dominate measurements made during a capacity test conducted over a couple of days, nor does it address the issues associated with various sources of structural shading that are mentioned above.

This paper develops "Approach 3" described in [@gosteinMeasuringIrradianceBifacial2021] that continues with the front-side POA irradiance measurement strategy by placing a rear-facing sensor in an outboard position. This allows the system capacity evaluation to be sensitive to deviations in practice from the modeled structural shading and allow sensor placement in a location that has smaller spatial gradients of irradiance. This reduces the sensitivity to sensor placement, and maintains a consistent boundary of evaluation with the monofacial capacity testing practices. For SAT arrays in particular, such a sensor will be exposed to higher irradiance levels than the average irradiance on the rear side of the array, so while it can be used for regression-based capacity estimation, such readings are not necessarily appropriate for computing performance ratio.


# Approach

This work focuses on deriving an estimate of the SAT outboard sensor reading from standard PVsyst modeling results, and illustrate obtaining the target power capacity estimate. 
...
The PVsyst $\mathrm{GlobBak}$ output is a naive candidate for comparison with sensors, but this quantity represents a _spatial average_ irradiance that is also _obstructed by structural shading_ objects. So an unobstructed average irradiance could be $\mathrm{GlobBak}+\mathrm{BackShd}$, except that being outboard in a SAT configuration exposes the sensor to additional irradiance.


## Outboard PVsyst equations

In the case of an outboard rear-facing irradiance sensor located as in @fig-offset at a distance $L$ along the torque-tube axis of a horizontal single-axis tracker (point $A$) at height $H$ at the end of the tracker closest to the equator, the edge of a block of trackers (perpendicular to point $B$) casts ($\overline{EB}$) an approximate "shade line" on the ground in the east-west direction (perpendicular to the paper at $D$), disregarding the sawtooth pattern arising from tracker tilt and the interruptions due to tracker row spacing. (Note that the planar shade angle $\phi$ in this diagram is different than the PVsyst $\mathrm{PhiAng}$) tracker rotation angle.)

![Height-offset diagram of SAT outboard sensor](../analysis/figures/bifi-tracker-sensor-geometry2.svg){#fig-offset}
...
\end{equation}
where $\mathrm{DifSBak}$ and $\mathrm{BmIncBk}$ are as compute by PVsyst.

# Results

[@fig-e-rear] compares the computed $E_\mathrm{rear}$ versus $\mathrm{GlobIncUnshd}=\mathrm{GlobBak}+\mathrm{BackShd}$ for a sample system with $H=2\mathrm{m}$ and $\mathrm{GCR}=0.493$ which should show agreement by aligning with the red 1:1 reference line. When the sensor offset position is far "Under" the tracker ($L=-100\mathrm{m}$) then the values are essentially identical. When the sensor position is "Adjacent" ($L=1\mathrm{m}$), then the estimated $E_\mathrm{rear}$ is approximately double the under-the-tracker value due to the GCR being close to $\sfrac{1}{2}$.
